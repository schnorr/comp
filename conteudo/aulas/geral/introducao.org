...
+ <2->Linguagens Simbólicas
  + Exemplo: Assembly
  + Add 4(0), #1
+ <3->\alert{Linguagens orientadas ao usuário}
...
+ Duas definições para \alert{compilador}
+ <1-> Forte: é um programa de computador que transforma uma
  \alert{linguagem de alto nível} qualquer em uma \alert{linguagem de máquina} de uma determinada arquitetura
...
*** Texto						     :B_column:BMCOL:
    ...
    + <2->Exemplo utilizando \alert{gcc}
...
+ Análise (front-end) \\
  \alert{Reconhecer} o que o usuário quer escrever
  + ``Palavras'' \rightarrow análise lexical
  + ``Frases'' \rightarrow análise sintática/semântica
  + Detecção de erros
    \vfill
+ <2->Síntese (back-end) \\
  \alert{Produzir} alguma coisa, em função da análise
...
+ Através do uso de \alert{Expressões Regulares}
...
Através do uso de \alert{Gramáticas Livres de Contexto}
...
+ Notação para gramáticas: \alert{BNF} (Backus-Naur Form) \\
  | *<comando>*   | \to | <while>                        |
...
#+END_EXAMPLE
** Árvore de derivação
+ Ilustra a \alert{derivação das regras} de uma gramática
...
** Análise Semântica
+ Avaliar a \alert{consistência semântica} do programa
+ Verificação de tipos
  + Métodos de coerção (caso a definição da linguagem autorisar)
+ Exemplo
** Geração de Código Intermediário
+ Usa a \alert{representação interna} do compilador
...
** Otimização de Código
 + Realizar \alert{otimizações} sobre o código intermediário
...
+ Guarda atributos das \alert{variáveis} e \alert{funções} do programa
...
\hspace{2.5cm}\includegraphics[width=.6\linewidth]{fases_compilacao_pratica.pdf}\hfill

** Geradores de Compiladores
+ Análise Léxica -- \alert{lex} e \alert{flex}
+ Análise Sintática -- \alert{yacc} e \alert{bison}
+ Gerador de Código
