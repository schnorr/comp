...
1. A ER é esta =(a*|(bc)*)=, podemos obtê-lá ao caminhar no autômato.
2. Não, pois se fosse construído utilizando os "blocos 
de construção de thompson" poderíamos observar transições vazias
entre tais blocos. Por exemplo, para uma concanetação como ab, 
teríamos um estado vazio ligando o estado que reconhece `a` e o 
que reconhece `b`, o que não ocorre entre os estados 4 e 5 neste
autômato, por exemplo.
3. Fech-\epsilon(0) = {0, 1, 2, 3, 6, 7, 14, 8, 9, 10, 12, 13} = A;
4. Mov(A, a) = {11};
   Fech-\epsilon(Mov(A, b)) = {0, 1, 2, 3, 6, 7, 14, 8, 9, 10, 12, 13,
   11};
...
2. A tabela criada para esta gramática foi:
...
tem mais de uma ação em uma célula, temos um conflito. Analisando
a linguagem poderíamos concluir também que não é pois está não 
respeita a regra 2, pois em S -> XP, ambos X e P podem se tornar
\epsilon.
...
Mesmo havendo conflitos na nossa linguagem, foi possível reconhecer a entrada
pois não encontramos nenhum deles no caminho.
...
1. Podemos utilizar [[https://github.com/schnorr/ellerre][ellerre]] para obter o automato.
   #+begin_src shell :results output :exports both
   echo 'S => a [ L ] | a ;
...
L => S L | S ;' > gramatica.ee
   ./lr0 < gramatica.ee > /dev/null
   ./firstfollow < gramatica.ee
   dot -Tpng LR0.dot -o resp_2024_1-E3-LR0.png
   #+end_src
...
2. Não, pois em LR0 estados de redução precisam estar sozinhos no estado.
 Podemos observar um conflito empilha-reduz no estado 2 e também no estado 4.
