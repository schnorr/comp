...
A entrega sempre é até as 23:59 do dia. O link para o formulário
único que permite realizar a entrega de todas as etapas encontra-se
abaixo.
...
para esse operação. Será gerado um arquivo chamado =etapaX.tgz=, o qual
...
Verifique as permissões de acesso ao arquivo (com o programa =chmod=
através de uma conexão com =sftp=). Recomenda-se que testem o acesso à
partir de um local externo utilizando o programa =wget=.
