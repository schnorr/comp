...
2. Não, pois se fosse construído utilizando os "blocos de construção de thompson" poderíamos observar transições vazias entre tais blocos. Por exemplo, para uma concanetação como ab, teríamos um estado vazio ligando o estado que reconhece `a` e o que reconhece `b`, o que não ocorre entre os estados 4 e 5 neste autômato, por exemplo.
