...
Sobre a ordem em que devemos imprimir os componentes de um
nodo. Devemos primeiro imprimir o label seguido dos filhos ou podemos
imprimir os filhos primeiro? Por exemplo:

deve ser assim?
#+BEGIN_EXAMPLE
0x556391aca3a0 [label="main"];
0x556391aca3a0, 0x556391aca910
0x556391aca3a0, 0x556391acb590
#+END_EXAMPLE
...
Fiquei confuso, pois tive a impressão pela resposta sua a uma de
minhas perguntas que o label deveria vir antes; porém, o script Python
que o senhor forneceu aparenta apenas aceitar quando o label vem
depois, dando KeyError caso o label apareça antes.

---

R: O script disponibilizado não será utilizado na avaliação
automática. O script que será utilizado na avaliação é robusto o
suficiente para aceitar em qualquer ordem.

*** Dúvida #8

Para a árvore gerada com o código a seguir, ficamos com uma dúvida:
quando há múltiplas inicializações numa linha de declarações locais,
como fica a árvore? A declaração local inteira é um comando simples?
Neste caso, a primeira atribuição teria quatro filhos, o
identificador, o valor de inicialização, a próxima inicialização e o
próximo comando simples. Isto está correto? Esta árvore representa
corretamente o código? Segue em anexo a árvore gerada em svg..

#+BEGIN_EXAMPLE
int main() {
    int a <=5, b <= 4, f, g, h, j, c <= 9, g <= 123;
    ghj = 4;
    return 0;
}
#+END_EXAMPLE
...
R: 
Já respondi essa dúvida no registro E3 D2 #7.6:
https://moodle.ufrgs.br/mod/page/view.php?id=2415127
https://moodle.ufrgs.br/mod/resource/view.php?id=2594529

*** Dúvida #9

 Analisando a saída do valgrind para o nosso programa, ele não está
 listando os bytes perdidos como mostra na definição do trabalho, mas
 está informando apenas que "All heap blocks were freed -- no leaks
 are possible". Esta é a saída completa do valgrind:

#+BEGIN_EXAMPLE
==4165== Memcheck, a memory error detector
==4165== Copyright (C) 2002-2017, and GNU GPL'd, by Julian Seward et al.
==4165== Using Valgrind-3.13.0 and LibVEX; rerun with -h for copyright info
==4165== Command: ./etapa3
==4165==
==4165==
==4165== HEAP SUMMARY:
==4165==     in use at exit: 0 bytes in 0 blocks
==4165==   total heap usage: 371 allocs, 371 frees, 99,310 bytes allocated
==4165==
==4165== All heap blocks were freed -- no leaks are possible
==4165==
==4165== For counts of detected and suppressed errors, rerun with: -v
==4165== ERROR SUMMARY: 0 errors from 0 contexts (suppressed: 0 from 0)
#+END_EXAMPLE

Isto indica que está tudo certo, não é? Há um problema por não mostrar
aquela listagem de bytes perdidos que o senhor colocou na seção B da
definição?

---

R: A sequência de teste com valgrind é a seguinte, assumindo que em
$test tem o nome do arquivo do teste.

#+begin_src bash
	valgrind ./etapa3 < $test 2>&1 | grep -v 0x | grep -A5 LEAK\ SUMMARY > valgrind.txt
...
Então é um ótimo sinal quando vocês obtiverem "no leaks are possible".

A versão do valgrind aqui é:
...
Tenho algumas dúvidas ainda sim! 
A seguinte iniciação de variáveis é válida?

#+BEGIN_EXAMPLE
int a <= 1, b, c <=2; 
#+END_EXAMPLE

Se sim, o correto seria botar apenas ~a <= 1~ e ~c<=2~ na arvore, certo? 

Além disso, podemos ter um no "ELSE", filho de um "IF" e com o filho
sendo o primeiro comando do bloco, ou devemos fazer um bloco "IF" sem
else e um "IF-ELSE"?

---

R: Já respondi a dúvida sobre as inicializações no registro E3 D2 #7.6:
https://moodle.ufrgs.br/mod/page/view.php?id=2415127
https://moodle.ufrgs.br/mod/resource/view.php?id=2594529

R: Não existem nós tipo ELSE na nossa especificação. Vejam a
   especificação no que diz respeito aos nós IF. Copio aqui para
   ajudar: "O comando if com else opcional deve ter pelo menos três
   filhos, um para a expressão, outro para o primeiro comando quando
   verdade, e o último -- opcional -- para o primeiro comando quando
   falso."

*** Dúvida #11

Primeiramente, agradeço pelo esclarecimento das dúvidas anteriores,
acabei esquecendo de mandar um e-mail em resposta pois estava ocupado
com o trabalho da disciplina. Acabou surgindo uma última dúvida, e
espero que não esteja tarde demais para saná-la. Funções podem receber
strings como argumento? Na etapa 2 diz que a lista de argumento é
composta por expressões, e se não me engano nessa etapa isso é dito
também. Como expressões só podem aceitar literais inteiros e float (e
agora true e false também), não adicionamos strings como argumentos,
ou mesmo char. Isso está correto?
